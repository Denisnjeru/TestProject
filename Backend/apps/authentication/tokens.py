...
class Token:
    ...
    def __init__(self, token=None, verify=True):
        ...
        if token is not None:
            ...
            self.payload = {settings.SIMPLE_JWT['TOKEN_TYPE_CLAIM']: self.token_type}
    ...
    def verify(self):
        ...
        if settings.SIMPLE_JWT['JTI_CLAIM'] not in self.payload:
    ...
    def verify_token_type(self):
        ...
        """
        try:
            token_type = self.payload[settings.SIMPLE_JWT['TOKEN_TYPE_CLAIM']]
        except KeyError:
    ...
    def set_jti(self):
        ...
        """
        self.payload[settings.SIMPLE_JWT['JTI_CLAIM']] = uuid4().hex
    ...
    def for_user(cls, user):
        ...
        """
        user_id = getattr(user, settings.SIMPLE_JWT['USER_ID_FIELD'])
        if not isinstance(user_id, int):
            user_id = str(user_id)

        token = cls()
        token[settings.SIMPLE_JWT['USER_ID_CLAIM']] = user_id
        token['username'] = user.username
        token['firstname'] = user.first_name
        token['lastname'] = user.last_name
        token['last_login'] = f''
        # token['profile'] = serializers.serialize('json', [user.user.get(), ])
        token['profile_pic'] = user.user.values('profile_pic').first()['profile_pic']
        token['notification_count'] = ''
        return token
...
class BlacklistMixin:
    ...
    if 'rest_framework_simplejwt.token_blacklist' in settings.INSTALLED_APPS:
        ...
        def for_user(cls, user):
            ...
            jti = token[settings.SIMPLE_JWT['JTI_CLAIM']]
            exp = token['exp']
...
class SlidingToken(BlacklistMixin, Token):
    token_type = 'sliding'
    lifetime = settings.SIMPLE_JWT['SLIDING_TOKEN_LIFETIME']
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.token is None:
            ...
            self.set_exp(
                settings.SIMPLE_JWT['SLIDING_TOKEN_REFRESH_EXP_CLAIM'],
                from_time=self.current_time,
                lifetime=settings.SIMPLE_JWT['SLIDING_TOKEN_REFRESH_LIFETIME'],
            )
